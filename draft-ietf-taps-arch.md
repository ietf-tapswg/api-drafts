...
Transport Properties can be specified to allow the application to configure the Transport System and express their requirements, prohibitions, and preferences. There are three kinds of Transport Properties: 

  * Selection Properties ({{preestablishment}}) that can only be specified as part of a Preconnection.
  * Connection Properties ({{preestablishment}}) that may be changed on the Connection.
  * and Message Properties ({{datatransfer}}) that can also be specified during data transfer to affect specific Messages.
