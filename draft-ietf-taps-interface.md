...
Type:
: Numeric (non-negative) or `Disabled` or `SysDefault`
...
could express `Disabled` and `SysDefault` in an environment-typical way, e.g., as a Union type or special value. Choosing `SysDefault` is a hint to the Transport Services system that it should
