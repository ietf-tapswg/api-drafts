...
- Singular Transmission: when this is true, the application requests to avoid transport-layer segmentation or network-layer fragmentation.  Some transport offer transport-layer segmentation, transports that offer transport-layer segmentation MUST fit this message into a single IP packet or fail.  Transport that do not offer transport-layer segmentation SHOULD NOT send packets greater the largest size the network layer can send in one IP packet, to avoid IP fragments {{?I-D.ietf-intarea-frag-fragile}}.  Endpoint fragmentation SHOULD be avoided and in IPv4 the don't fragment bit SHOULD be set (SET_DF in {{!RFC8304}}).
