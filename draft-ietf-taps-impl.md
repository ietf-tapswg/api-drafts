...
There are many common user scenarios that can lead to a path becoming temporarily unavailable, and then recovering before the transport protocol hits a timeout error. These are particularly common with mobile devices. Examples include: an Ethernet cable becoming unplugged and then plugged back in; a device losing Wi-Fi signal while a user in an elevator, and reattaching when the user leaves the elevator; and a user losing radio signal while riding a train through a tunnel. If the device is able to rejoin a network with the same IP address, a stateful transport connection can generally resume. Thus, while it is useful for an application to be aware of a temporary loss of connectivity, the Transport Services implementation should not aggressively close connections in these scenarios.
